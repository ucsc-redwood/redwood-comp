 ...
 protected:
  void SetUp(benchmark::State&) override {
    ...
    tmp_storage =
        std::make_unique<vulkan::TmpStorage>(engine->get_mr(), kInputSize);
    dispatcher =
        std::make_unique<vulkan::Dispatcher>(*engine, *app_data, *tmp_storage);
...
#define REGISTER_BENCHMARK(NAME, ITERATIONS)        \
  ...
  REGISTER_BENCHMARK(run_stage1, 100);
  REGISTER_BENCHMARK(run_stage2, 10);
  REGISTER_BENCHMARK(run_stage3, 100);
  REGISTER_BENCHMARK(run_stage4, 100);
  REGISTER_BENCHMARK(run_stage5, 100);
  REGISTER_BENCHMARK(run_stage6, 100);
  REGISTER_BENCHMARK(run_stage7, 100);
